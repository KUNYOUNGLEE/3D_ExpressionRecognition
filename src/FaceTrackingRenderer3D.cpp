...
template <int dim> double FaceTrackingRenderer3D::CalculateDistance(PXCPoint3DF32 p1, PXCPoint3DF32 p2)
{
	if (dim == 2)
...
void FaceTrackingRenderer3D::DrawLandmark(PXCFaceData::Face* trackedFace)
{
	...
	for (int j = 0; j < numPoints; j++)
	{
		...
		if(ProjectVertex(points[j].world, ix, iy, 1))
		{
			...
			{
				DrawPoint(dc2, RGB(255, 255, 255), my_round(ix), my_round(iy), 4);
			}
		}
	}
